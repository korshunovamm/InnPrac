...
class Buy(unittest.TestCase):
    ...
    def test_buy_credit(self):
        ...
        pl = game.new_lab("test", "test")
        pl.money = 10000
        eq = game.buy_equipment(pl.get_uuid(), "auto", "blue", True)[0]
    ...
    def test_buy_to_much_credit(self):
        ...
        self.assertEqual(len(pl.equipments), 20)
        self.assertEqual(10000 - 16 * 4, pl.money)
...
class Move(unittest.TestCase):
    ...
    def test_move_to_room(self):
        ...
        eq = game.buy_equipment(pl.get_uuid(), "pre_analytic", None, False)[0]
...
class BuyReagents(unittest.TestCase):
    ...
    def test_buy_reagents(self):
        game = Game()
        pl_uuid = game.new_lab("Игрок", "Пароль").get_uuid()
        ro_uuid = game.buy_room(pl_uuid).get_uuid()
        eq_uuid = game.buy_equipment(pl_uuid, "auto", "blue", False)[0].get_uuid()
        for x in range(4):
